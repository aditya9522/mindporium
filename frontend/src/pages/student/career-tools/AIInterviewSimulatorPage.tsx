...
import { Bot, BrainCircuit, CheckCircle2, ClipboardCheck, MessageSquare, Send, Sparkles, Timer } from 'lucide-react';
                        ...
                        <div className="mt-4 space-y-4">
                            ...
                            <Field label="Job Description">
                                <textarea
                                    ...
                                    rows={7}
                                    placeholder="Optional, but improves question relevance..."
                            ...
                            <div className="space-y-5">
                                <div className="flex flex-col gap-4 border-b border-gray-100 pb-5 dark:border-gray-800 lg:flex-row lg:items-start lg:justify-between">
                                    <div>
                                        ...
                                        <h2 className="mt-2 text-2xl font-bold leading-tight text-gray-900 dark:text-white">{selectedQuestion.question}</h2>
                        ...
                        <div className="rounded-lg border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
                            <div className="flex flex-col gap-4 border-b border-gray-100 pb-4 dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    ...
                                    <p className="mt-1 text-sm text-gray-500">{feedback.verdict}</p>
                                </div>
                            ...
                            <div className="mt-5 rounded-lg bg-gray-50 p-4 dark:bg-gray-800/60">
                                ...
                                <p className="mt-2 text-sm leading-6 text-gray-700 dark:text-gray-300">{feedback.betterAnswer}</p>
                            </div>
                            {feedback.followUpQuestion && (
                                <div className="mt-4 rounded-lg border border-primary-100 bg-primary-50 p-4 dark:border-primary-900 dark:bg-primary-950/30">
                                    ...
                                    <p className="mt-2 text-sm font-semibold text-primary-950 dark:text-primary-100">{feedback.followUpQuestion}</p>
                                </div>
                            )}
                        </div>
                    )}
...
const AIAvatar = ({ active, thinking, large = false }: { active: boolean; thinking: boolean; large?: boolean }) => (
    <div className={`relative flex ${large ? 'h-24 w-24' : 'h-16 w-16'} shrink-0 items-center justify-center rounded-full border border-primary-200 bg-white shadow-sm dark:border-primary-900 dark:bg-gray-900`}>
        <div className={`${large ? 'h-16 w-16' : 'h-11 w-11'} flex items-center justify-center rounded-full bg-primary-600 text-white`}>
        ...
        {(active || thinking) && (
            <div className="absolute -bottom-1 flex items-end gap-0.5 rounded-full bg-white px-2 py-1 shadow-sm dark:bg-gray-800">
                {[0, 1, 2].map((item) => (
                    <span
                        key={item}
                        className={`block w-1 rounded-full bg-primary-500 ${thinking ? 'animate-pulse' : ''}`}
