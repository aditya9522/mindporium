        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-950 py-8 transition-colors duration-300">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="w-full space-y-8">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="relative group">
                                <select
                                    ...
                                    className="appearance-none bg-white/80 dark:bg-gray-900/80 backdrop-blur-md pl-6 pr-12 py-4 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 font-semibold hover:border-indigo-400 focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all cursor-pointer shadow-sm hover:shadow-md min-w-[280px]"
                                >
                                    {classrooms.map(cls => (
                    ...
                    {/* Search & Filter */}
                    <div className="flex flex-col sm:flex-row items-center gap-4 bg-white dark:bg-gray-900 p-2 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-[0_4px_20px_rgb(0,0,0,0.03)] transition-colors duration-300">
                        <div className="flex-1 w-full flex items-center gap-3 pl-4">
                            ...
                            <input
                                ...
                                className="flex-1 py-3 outline-none text-gray-700 dark:text-gray-200 font-medium placeholder:text-gray-400 bg-transparent"
                            />
                        </div>
                                        ...
                                        filteredAttendance.map((record) => (
                                            <tr key={record.id} className="group hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                                                <td className="px-8 py-5">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            <div className="font-bold text-gray-900 dark:text-gray-100 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors truncate">{record.user?.full_name || 'Unknown User'}</div>
                                                            <div className="flex items-center gap-2">
                                                                <div className="text-gray-500 dark:text-gray-400 text-xs font-medium truncate">{record.user?.email || 'No email provided'}</div>
                                                                {record.user?.email && (
                                                                    <button
                                                                        onClick={() => {
                                                ...
                                                ${record.status === 'present'
                                                            ...
                                                            : 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 border border-amber-100/50 dark:border-amber-800/50'}`}>
                                                ...
                                                <td className="px-8 py-5">
                                                    {record.left_at ? (
                                                        <div className="flex items-center gap-2 bg-gray-100/50 dark:bg-gray-800/50 px-3 py-1.5 rounded-lg w-fit text-sm font-semibold text-gray-600 dark:text-gray-300">
                                                ...
                                                <td className="px-8 py-5">
                                                    <div className="flex flex-col gap-1.5">
                                                        {record.device_info && (
                                                            <div className="flex items-center gap-2 text-xs font-medium text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 px-2.5 py-1 rounded-md w-fit" title={record.device_info}>
                                                        ...
                                                        {record.ip_address && (
                                                            <div className="flex items-center gap-2 text-xs font-mono text-gray-400 dark:text-gray-500 pl-1">
                                                                <Globe className="w-3 h-3 opacity-50" />
                                                                {record.ip_address}
                                                            </div>
                                                        )}
                                                        {!record.device_info && !record.ip_address && (
                                                            <span className="text-gray-300 dark:text-gray-600 text-xs italic">No device data</span>
                                                        )}
                                                    </div>
                                                </td>
                                            </tr>
